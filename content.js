...
function replaceImages(images) {
    for (let img of images) {
        if (!img.classList.contains('elon-replaced')) {
            ...
            // Replace image and remove unwanted attributes
            ...
            // Add hover effect to restore the original image
            img.addEventListener('mouseover', () => img.src = originalSrc);
            img.addEventListener('mouseout', () => img.src = replaceImageUrl);

            console.log("Image replaced, original will show on hover:", img);
        }
    }
}

function replaceSources(sources) {
    for (let source of sources) {
        if (!source.classList.contains('elon-replaced')) {
...
function maskElonImages() {
    ...
    const sources = Array.from(document.getElementsByTagName('source')).filter(source => 
        [...source.attributes].some(attr => regex.test(attr.value))
    );
...
function maskElonContent() {
    ...
    for (let element of elements) {
        ...
        const foundInAttributes = attributesToCheck.some(attr => regex.test(element.getAttribute(attr) || ''));
    ...
    for (let link of links) {
        if (regex.test(link.textContent) || regex.test(link.title)) {
            ...
            link.textContent = "████";
            console.log("Link content replaced:", link);
        }
    }
}
